...
import type { Prepa } from "../../types/prepa";

/* ------------------------------------------------------------------ */
/* 🔧 Helpers utilitaires (identiques à ExportButtonAteliersTRE) */
/* ------------------------------------------------------------------ */
function getFilenameFromDisposition(disposition?: string | null, fallback = "export_prepa.xlsx") {
...
/* ------------------------------------------------------------------ */
/* 🧩 Composant principal : Export Prépa */
/* ------------------------------------------------------------------ */
type Props = {
  data: Prepa[];
  selectedIds: number[];
...
export default function ExportButtonPrepa({ data, selectedIds }: Props) {
  ...
  const handleExport = async () => {
    if (total === 0) {
      toast.warning("Aucune donnée Prépa à exporter.");
      return;
    }
    ...
    try {
      ...
      // 🔹 Base d’URL d’export pour Prépa
      const base = "/prepa/export-xlsx/";
      ...
      const filename = getFilenameFromDisposition(disposition, "prepa_export.xlsx");
      ...
      toast.success(
        selectedCount > 0
          ? `Export XLSX des ${selectedCount} sélection(s) Prépa prêt.`
          : `Export XLSX du jeu filtré Prépa prêt.`
      );

      setShowModal(false);
    } catch (e: unknown) {
      const msg = getErrorMessage(e) || "Erreur lors de l’export Prépa.";
      toast.error(msg);
    } finally {
      setBusy(false);
    }
  ...
  return (
    <>
      <Button
        ...
        aria-label={`Exporter ${selectedCount || total} activité(s) Prépa`}
        title={
          total === 0
            ? "Aucune activité Prépa à exporter"
            : `Exporter ${selectedCount || total} activité(s) Prépa`
        }
      >
        {busy ? "⏳ " : "⬇️ "}
      ...
      <Dialog open={showModal} onClose={closeModal} maxWidth="sm" fullWidth>
        <DialogTitle>Exporter les activités Prépa</DialogTitle>
