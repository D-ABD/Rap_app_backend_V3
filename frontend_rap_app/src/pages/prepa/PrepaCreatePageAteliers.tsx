...
export default function PrepaCreatePageAteliers() {
  ...
  const handleSubmit = async (values: Partial<Prepa>) => {
    try {
      ...
      const parsed = data ? extractPrepaApiMessage(data, { rawFields: ["participations_prepa"] }) : null;
  ...
  if (loadingMeta) {
    return (
      <PageTemplate title="➕ Nouvel atelier Prépa" centered>
        <CircularProgress />
  ...
  return (
    <PageTemplate title="➕ Nouvel atelier Prépa" backButton onBack={() => navigate(-1)}>
