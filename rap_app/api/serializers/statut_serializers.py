...
class StatutSerializer(serializers.ModelSerializer):
    ...
    nom = serializers.ChoiceField(
        choices=Statut.STATUT_CHOICES, help_text="Nom interne du statut (valeurs définies en base)."
    )
    couleur = serializers.CharField(
        required=False, allow_blank=True, help_text="Couleur hexadécimale du badge (#RRGGBB)."
    )
    description_autre = serializers.CharField(
        required=False, allow_blank=True, help_text="Texte personnalisé pour le statut 'Autre'."
    )
    ...
    def validate(self, data):
        ...
        if nom == Statut.AUTRE and not description_autre:
            raise serializers.ValidationError({"description_autre": "Le champ est requis pour un statut 'Autre'."})

        if couleur and not re.match(r"^#[0-9A-Fa-f]{6}$", couleur):
            raise serializers.ValidationError({"couleur": "La couleur doit être au format hexadécimal (#RRGGBB)."})
...
class StatutChoiceSerializer(serializers.Serializer):
    ...
    value = serializers.CharField(help_text="Identifiant interne du statut (ex: 'non_defini')")
    label = serializers.CharField(help_text="Libellé lisible du statut (ex: 'Non défini')")
    default_color = serializers.CharField(help_text="Couleur hexadécimale par défaut du statut (ex: '#FFEB3B')")
