...
class LogUtilisateurModelTest(BaseModelTestSetupMixin, TestCase):
    ...
    def test_log_system_action(self):
        system_user = CustomUser.objects.create_user(
            email="system@example.com", password="testpass", first_name="System", last_name="Bot"
        )
        log = LogUtilisateur.log_system_action(
            action=LogUtilisateur.ACTION_LOGIN, user=system_user, details="Connexion système réussie"
        )
        self.assertIsNotNone(log)
    ...
    def test_to_dict_output(self):
        log = LogUtilisateur.log_action(
            instance=self.formation, action=LogUtilisateur.ACTION_UPDATE, user=self.user, details="Mise à jour test"
        )
        data = log.to_dict()
    ...
    def test_log_with_multiple_sensitive_fields(self):
        ...
        log = LogUtilisateur.log_action(
            instance=self.formation, action=LogUtilisateur.ACTION_UPDATE, user=self.user, details=details
        )
    ...
    def test_log_with_user_none(self):
        ...
        log = LogUtilisateur.log_action(
            instance=self.formation, action=LogUtilisateur.ACTION_VIEW, user=None, details="Vue automatique"
        )
        self.assertIsNotNone(log)
    ...
    def test_logs_ordering(self):
        ...
        log1 = LogUtilisateur.log_action(
            instance=self.formation, action=LogUtilisateur.ACTION_CREATE, user=self.user, details="Premier log"
        )
        ...
        log2 = LogUtilisateur.log_action(
            instance=self.formation, action=LogUtilisateur.ACTION_EXPORT, user=self.user, details="Deuxième log"
        )
        ...
        logs = list(
            LogUtilisateur.objects.filter(content_type=content_type, object_id=self.formation.pk).order_by(
                "-created_at"
            )[:2]
        )
    ...
    def test_log_with_invalid_input(self):
        ...
        log_none = LogUtilisateur.log_action(instance=None, action=LogUtilisateur.ACTION_VIEW, user=self.user)
        self.assertIsNone(log_none)

        # Test avec une action vide
        log_empty = LogUtilisateur.log_action(instance=self.formation, action="", user=self.user)
    ...
    def test_log_different_content_types(self):
        ...
        log_centre = LogUtilisateur.log_action(
            instance=self.centre, action=LogUtilisateur.ACTION_VIEW, user=self.user, details="Vue du centre"
        )
        ...
        log_statut = LogUtilisateur.log_action(
            instance=self.statut, action=LogUtilisateur.ACTION_VIEW, user=self.user, details="Vue du statut"
        )
        ...
        self.assertEqual(centre_logs.count(), 1, f"Attendu 1 log pour le Centre, trouvé {centre_logs.count()}")
