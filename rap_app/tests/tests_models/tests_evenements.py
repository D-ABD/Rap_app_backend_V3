"""Tests relatifs a evenements."""
from django.core.exceptions import ValidationError
from django.utils import timezone

from ...models import Centre, Evenement, Formation, Statut, TypeOffre
...
class EvenementModelTest(BaseModelTestSetupMixin):
    ...
    def test_description_autre_required_if_type_autre(self):
        evt = Evenement(type_evenement=Evenement.TypeEvenement.AUTRE, event_date=timezone.now().date())
        with self.assertRaises(ValidationError):
            evt.full_clean()

    def test_participation_rate_none_when_missing_data(self):
        evt = Evenement(type_evenement=Evenement.TypeEvenement.FORUM, participants_prevus=None, participants_reels=5)
    ...
    def test_temporal_status_past_today_soon_future(self):
        ...
        for expected_status, date in cases.items():
            evt = self.create_instance(
                Evenement, formation=self.formation, type_evenement=Evenement.TypeEvenement.JOB_DATING, event_date=date
            )
            self.assertEqual(evt.get_temporal_status(), expected_status)

    def test_participation_status_levels(self):
        evt_high = self.create_instance(
            Evenement,
            formation=self.formation,
            type_evenement=Evenement.TypeEvenement.FORUM,
            participants_prevus=10,
            participants_reels=10,
        )
        self.assertEqual(evt_high.participation_status, "success")
        evt_mid = self.create_instance(
            Evenement,
            formation=self.formation,
            type_evenement=Evenement.TypeEvenement.FORUM,
            participants_prevus=10,
        ...
        evt_low = self.create_instance(
            Evenement,
            formation=self.formation,
            type_evenement=Evenement.TypeEvenement.FORUM,
            participants_prevus=10,
            participants_reels=3,
        )
